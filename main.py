...
5. 小保底如果歪出往期up角色，计入意外up统计。（对有规划的下池无任何作用）
...
6. 本期卡池满30抽送10抽（特殊十连抽，不能单抽，不计入保底，出货不清空小保底和大保底，不干涉小保底跨池继承和大保底跨池清空）
7. 前一个卡池抽满60抽，本期赠送正常10连抽（不能单抽，计入保底和总抽数计数，不算入实际投入抽数）
8. 赠送10抽福利如果是现时的，则必须一次性抽完，不能中途停止，相当于十连抽。不限时则可以在任意池子使用，要求在60赠抽和30赠抽消耗完后使用。
